...
class Cube(object):
    ...
    face_corr_dict = {'U': 'U','D':'D', 'F':'F','B':'B','R':'R','L':'L'}
    ...
    def __init__(self, N, init_colors=None, whiteplastic=False):
        """
        (see above)
        """
        self.N = N
        self.stickers = np.array([np.tile(i, (self.N, self.N)) for i in range(6)])
        # self.stickers[0][0] = [1, 1, 1]
        if not (init_colors is None):
            self.stickers = np.copy(init_colors)
        print('stickers', self.stickers)
    ...
    def roll(self):
        self.turn('F', -1)
        tmp = {}
        tmp['F'] = self.face_corr_dict['F']
        tmp['B'] = self.face_corr_dict['B']
        tmp['U'] = self.face_corr_dict['R']
        tmp['L'] = self.face_corr_dict['U']
        tmp['D'] = self.face_corr_dict['L']
        tmp['R'] = self.face_corr_dict['D']
        self.face_corr_dict = tmp

    def turn_U_CW(self):
        self.move('D', 0, 1)
        self.turn('U', 1)
        tmp = {}
        tmp['U'] = self.face_corr_dict['U']
        tmp['D'] = self.face_corr_dict['D']
        tmp['F'] = self.face_corr_dict['R']
        tmp['L'] = self.face_corr_dict['F']
        tmp['B'] = self.face_corr_dict['L']
        tmp['R'] = self.face_corr_dict['B']
        self.face_corr_dict = tmp
    ...
    def turn_U_CCW(self):
        ...
        tmp['U'] = self.face_corr_dict['U']
        tmp['D'] = self.face_corr_dict['D']
        tmp['F'] = self.face_corr_dict['L']
        tmp['L'] = self.face_corr_dict['B']
        tmp['B'] = self.face_corr_dict['R']
        tmp['R'] = self.face_corr_dict['F']
        self.face_corr_dict = tmp
    ...
    def turn_U_180(self):
        ...
        tmp['U'] = self.face_corr_dict['U']
        tmp['D'] = self.face_corr_dict['D']
        tmp['F'] = self.face_corr_dict['B']
        tmp['L'] = self.face_corr_dict['R']
        tmp['B'] = self.face_corr_dict['F']
        tmp['R'] = self.face_corr_dict['L']
        self.face_corr_dict = tmp
    
    def switch_front_face(self):
        self.turn('U', -1)
        tmp = {}
        tmp['U'] = self.face_corr_dict['U']
        tmp['D'] = self.face_corr_dict['D']
